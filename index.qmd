...
K-Fold Cross Validation involves splitting the data into "k" number of
folds. The first formula shown here computes log loss for each fold which then is summed up to get an overall error measure for each lambda. K-Fold Cross Validation was used to determine the best lambda value, that results in the minimum mean cross-validated error (log loss).
