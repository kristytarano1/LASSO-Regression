...
LASSO regression was conducted as a means to tune our model and to minimize variables with low impact to our outcome variable. To develop a baseline for performing LASSO Regression, we began with a binary logistic regression model. The binary logistic regression model is formulated as shown below: 
...
LASSO regression is an optimizing feature selection technique used to assist with common regression problems such as the aforementioned reduction of overfitting through regularization. The purpose of using a regularization technique such as LASSO regression is to introduce a  penalization parameter known as  L1. L1 is determined by multiplying Lambda with the sums of the absolute values of the estimated parameters, demonstrated by the portion of the formula shown here: 


$$ \lambda \sum_{j=1}^p |\beta_j| $$


K-Fold Cross Validation involves splitting the data into "n" number of folds. The first formula shown here computes log loss which is summed with the errors for "n" number of folds. K-Fold Cross Validation also can interpret what variables account for variability in the model.  
...
coef(gene_model)
=======
#Coefficients and stats of the model
coef(gene_model)
...
#y_prediction <- predict(gene_model, s=min_lambda, newx = x_test, type = "response")



#CONFUSION MATRIX BELOW 

#Creating the prediction probability classes
pred_prob <- predict(gene_model, newx = x_test, s = min_lambda, type = "class")

#Organizes the predictions based on threshold 
pred_class <- ifelse(pred_prob > 0.05, 1, 0)

#Builds the confusion matrix with the organized predictions and response variable from test data
...
#Calculates the total true predictions(+ and -) over the sum of total predictions
accuracy <- sum(diag(confusion_matrix)) / sum(confusion_matrix)

# Prints the accuracy decimal
print(paste("Accuracy:", round(accuracy, 1)))

#Checking the accuracy of the positive predictions
precision <- diag(confusion_matrix) / rowSums(confusion_matrix)
